...
class Star
{
	...
	function init ( n  )
	{
		...
		pos_y = (rand()*480/RAND_MAX);
		pos_z = 100;
		speed = (rand()*5/RAND_MAX)+1;
		// create the star object for x & y 2d projection
		img = fe.add_image( "star.png", ((( pos_x-320) * 100 ) / pos_z ) +320, ((( pos_y-240) * 100 ) / pos_z ) +240 );
        BlackZones = {};
	}
	...
	function move()
	{
        ...
        if( pos_z <=0 ) {
            pos_z = 100;
            pos_x = (rand()*1024/RAND_MAX);
            pos_y = (rand()*768/RAND_MAX);
            speed = (rand()*3/RAND_MAX)+1;
        }
        // change image coord with the news one calculated with a 3d -> 2d projection
        local tmp_x = ((( pos_x-320) * 100 ) / pos_z ) +865;
        local tmp_y = ((( pos_y-240) * 100 ) / pos_z ) +400;
